...
# wait-for-it scripts.
(/usr/bin/wait-for localhost:8585 -- env PUBSUB_EMULATOR_HOST=localhost:8585 /usr/bin/pubsubc -debug; nc -lkp 8682 >/dev/null) &

# Start the PubSub emulator in the foreground.
gcloud beta emulators pubsub start --host-port=0.0.0.0:8585 --verbosity=debug "$@"
